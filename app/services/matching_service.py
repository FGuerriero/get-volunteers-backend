...
class MatchingService:
    ...
    **Output Format:**
    ...
    -   `volunteer_id` (for analyze_and_match) or `need_id` (for analyze_volunteer_against_all_needs): The integer ID of the matched entity.
    -   `match_details`: A string explaining *why* this volunteer/need is a good fit, specifically mentioning how their skills, interests, or 'about me' section align.
    ...
    [
        {{
            "need_id": 789,
            "match_details": "Volunteer's 'Gardening' skill is perfect for the 'Community Garden Cleanup' need, and their 'Environmental protection' interest aligns."
        }},
        {{
            "need_id": 101,
            "match_details": "Volunteer's 'Communication' skill is suitable for the 'Public Speaking Event' need, and their 'Community outreach' interest is a strong fit."
        }}
    ]
    ```
    """
    ...
    async def analyze_volunteer_against_all_needs(self, volunteer: models.Volunteer, all_needs: List[models.Need]):
        """
        ...
        need_info = "\n".join([
            f"- ID: {n.id}, Title: {n.title}, Description: {n.description}, Required Skills: {n.required_skills or 'None'}"
            for n in all_needs
        ])

        prompt = f"""
        You are an expert volunteer matching specialist. Your task is to analyze a specific volunteer's profile and a list of available needs, and identify the best fits for this volunteer based on their skills and interests.
            ...
            for match_data in gemini_response:
                ...
                if isinstance(need_id, int) and isinstance(match_details, str):
                    if crud_need.get_need(self.db, need_id):
                        ...
                        print(f"Warning: Gemini suggested non-existent need ID {need_id} for Volunteer ID {volunteer.id}")
                else:
                    print(f"Warning: Gemini returned invalid match data format for Volunteer ID {volunteer.id}: {match_data}")
        else:
